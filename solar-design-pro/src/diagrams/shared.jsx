...
// PV Group Icon — renders a single group's module layout
// group: {ori, ...}, positions: [{x,y,id}...], md: {lm, wm}
export const pvGroupIcon = (group, positions, md, opts) => {
  const { maxW = 120, maxH = 50 } = opts || {};
  if (!group || !positions || positions.length === 0 || !md) {
    return (
      <g>
  ...
  positions.forEach(p => {
    ...
    maxX_ = Math.max(maxX_, p.x + mw); maxY_ = Math.max(maxY_, p.y + mh);
  });

  const bboxW = maxX_ - minX || 1, bboxH = maxY_ - minY || 1;
  const scale = Math.min(maxW / bboxW, maxH / bboxH) * 0.88;
  const actualW = bboxW * scale, actualH = bboxH * scale;
  const offX = (maxW - actualW) / 2, offY = (maxH - actualH) / 2;

  return (
    <g>
      <rect width={maxW} height={maxH} rx={2} fill="#1e40af" opacity={0.1} stroke="#1e3a8a" strokeWidth={0.8} />
      {positions.map((p, i) => (
        <rect key={i} x={offX + (p.x - minX) * scale} y={offY + (p.y - minY) * scale}
          width={mw * scale} height={mh * scale}
          fill="#1e40af" opacity={0.6} stroke="#1e40af" strokeWidth={0.4} rx={0.5} />
      ))}
    </g>
  );
};
