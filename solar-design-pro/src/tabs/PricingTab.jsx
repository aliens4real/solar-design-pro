...
function loadEmployees() {
  try { const s = localStorage.getItem(EMP_KEY); return s ? JSON.parse(s) : null; }
  catch { return null; }
}

// ── Task Definitions ──
const TASKS = [
  { id: "mod_install",  nm: "Module Installation",         fn: (m, ni, mt) => m * 0.5 },
  { id: "racking",      nm: "Racking & Mounting",          fn: (m, ni, mt) => m * 0.4 },
  { id: "inv_install",  nm: "Inverter Installation",       fn: (m, ni, mt) => ni * 2 },
  { id: "dc_wiring",    nm: "DC Wiring",                   fn: (m, ni, mt) => m * 0.15 + 2 },
  { id: "ac_wiring",    nm: "AC Wiring & Interconnection", fn: (m, ni, mt) => 4 },
  { id: "grounding",    nm: "Grounding & Bonding",         fn: (m, ni, mt) => 2 },
  { id: "labeling",     nm: "Labeling & Commissioning",    fn: (m, ni, mt) => 2 },
  { id: "site_prep",    nm: "Site Prep & Cleanup",         fn: (m, ni, mt) => 2 },
];

const MT_MULT_TASKS = new Set(["mod_install", "racking"]);

function calcTaskHours(totalMods, ni, mt, taskOvr) {
  const mult = { roof: 1.0, ground: 1.2, carport: 1.3 }[mt] || 1.0;
  return TASKS.map(t => {
    const auto = t.fn(totalMods || 0, ni, mt);
    const adjusted = MT_MULT_TASKS.has(t.id) ? auto * mult : auto;
    const hrs = taskOvr[t.id] != null ? taskOvr[t.id] : adjusted;
    return { ...t, auto: +adjusted.toFixed(1), hrs: +hrs };
  });
}

// ── Grant Presets (OH/PA common programs) ──
// tp: "pct" = % of system cost, "wpv" = $/watt, "fix" = fixed dollar
const GRANT_PRESETS = [
  { id: "itc_comm",  nm: "Commercial ITC (48E)",         tp: "pct", val: 30,   cat: "federal",  note: "Begin construction by 7/4/2026" },
  { id: "itc_dom",   nm: "ITC Domestic Content Bonus",   tp: "pct", val: 10,   cat: "federal",  note: "Adder on top of base ITC" },
  { id: "itc_ec",    nm: "ITC Energy Community Bonus",   tp: "pct", val: 10,   cat: "federal",  note: "Adder for energy community sites" },
  { id: "usda_reap", nm: "USDA REAP Grant",              tp: "pct", val: 25,   cat: "federal",  note: "Rural ag/small business, up to $1M" },
  { id: "pa_sep",    nm: "PA Solar Energy Program",      tp: "wpv", val: 1.50, cat: "state_pa", note: "Commercial/nonprofit, up to $1M" },
  { id: "pa_sba",    nm: "PA Small Business Advantage",  tp: "fix", val: 9750, cat: "state_pa", note: "50-80% up to $12K" },
  { id: "peco",      nm: "PECO Solar Rebate",            tp: "fix", val: 500,  cat: "utility",  note: "Residential, through 4/2026" },
  { id: "ppl_comm",  nm: "PPL Commercial Rebate",        tp: "wpv", val: 0.08, cat: "utility",  note: "$/kWh 1st yr — entered as $/W equiv" },
  { id: "oh_prop",   nm: "OH Property Tax Exemption",    tp: "fix", val: 0,    cat: "state_oh", note: "Value varies by mill rate — enter est." },
  { id: "other",     nm: "Other Grant / Rebate",         tp: "fix", val: 0,    cat: "other",    note: "" },
];

const CAT_LABELS = { federal: "Federal", state_pa: "Pennsylvania", state_oh: "Ohio", utility: "Utility", other: "Other" };
...
export default function PricingTab({ pk, dsg, pj, totalMods, totalKw, md, iv, ivs, logo }) {
  ...
  const [taskOvr, setTaskOvr] = useState(saved?.taskOvr ?? {});
  const [grants, setGrants] = useState(saved?.grants ?? []);

  // Employee roster
  const [employees, setEmployees] = useState(() => loadEmployees() ?? DEF_EMPLOYEES);
  const [rosterOpen, setRosterOpen] = useState(false);

  // Auto-save pricing
  useEffect(() => {
    const t = setTimeout(() => {
      try { localStorage.setItem(PRC_KEY, JSON.stringify({ matMkup, labMkup, team, dayOverride: dayOvr, taskOvr, grants })); } catch {}
    }, 500);
    return () => clearTimeout(t);
  }, [matMkup, labMkup, team, dayOvr, taskOvr, grants]);

  // Auto-save employees
  useEffect(() => {
    const t = setTimeout(() => {
      try { localStorage.setItem(EMP_KEY, JSON.stringify(employees)); } catch {}
    }, 500);
    return () => clearTimeout(t);
  }, [employees]);
  ...
  const addEmployee = () => {
    const sk = SKILLS[4];
    setEmployees(e => [...e, { id: Date.now(), nm: `New Employee`, skill: sk.id, rate: sk.rate }]);
  };
  const updEmployee = (id, k, v) => setEmployees(e => e.map(emp => emp.id === id ? { ...emp, [k]: v } : emp));
  const delEmployee = (id) => setEmployees(e => e.filter(emp => emp.id !== id));
  const addToTeam = (emp) => {
    setTeam(t => [...t, { id: Date.now(), nm: emp.nm, skill: emp.skill, rate: emp.rate }]);
  };

  // Grant CRUD
  const addGrant = (preset) => {
    setGrants(g => [...g, { id: Date.now(), nm: preset.nm, tp: preset.tp, val: preset.val, note: preset.note }]);
  };
  const updGrant = (id, k, v) => setGrants(g => g.map(gr => gr.id === id ? { ...gr, [k]: v } : gr));
  const delGrant = (id) => setGrants(g => g.filter(gr => gr.id !== id));
  ...
  // Task-level hour breakdown
  const taskRows = useMemo(() => calcTaskHours(totalMods, ni, mt, taskOvr), [totalMods, ni, mt, taskOvr]);
  const totalHours = taskRows.reduce((s, t) => s + t.hrs, 0);
  const estDays = Math.max(1, Math.ceil(totalHours / (HRS * Math.max(1, team.length))));
  ...
  const grantRows = useMemo(() => grants.map(g => {
    let amt = 0;
    if (g.tp === "pct") amt = totalCost * (g.val / 100);
    else if (g.tp === "wpv") amt = g.val * totalKw * 1000;
    else amt = g.val;
    return { ...g, amt: Math.max(0, amt) };
  }), [grants, totalCost, totalKw]);
  const grantTotal = grantRows.reduce((s, g) => s + g.amt, 0);
  const netCost = totalCost - grantTotal;

  const pricePerWatt = totalKw > 0 ? totalCost / (totalKw * 1000) : 0;
  const netPerWatt = totalKw > 0 ? netCost / (totalKw * 1000) : 0;
...
${grantRows.length > 0 ? `
<h2>Incentives & Grants</h2>
<table>
  <thead><tr><th>Incentive</th><th class="r">Amount</th></tr></thead>
  <tbody>
    ${grantRows.map(g => `<tr><td>${g.nm}</td><td class="r" style="color:#16a34a">-$${g.amt.toFixed(2)}</td></tr>`).join("\n    ")}
  </tbody>
  <tfoot>
    <tr class="total-row" style="border-top-color:#16a34a"><td style="color:#16a34a">NET COST</td><td class="r" style="color:#16a34a">$${netCost.toFixed(2)}</td></tr>
  </tfoot>
</table>
` : ""}
...
<div style="margin-bottom:16px">
  ...
  ${grantTotal > 0 ? `<span class="metric">Net $/Watt: <span style="color:#16a34a">$${netPerWatt.toFixed(2)}</span></span>` : ""}
    ...
    <div style={{ maxWidth: 1000, margin: "0 auto" }} className="fi">
      ...
      {pk.length === 0 ? (
        ...
        <div style={{ ...cd, marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 14 }}>
            <div>
              ...
              <div style={{ fontFamily: ff, fontSize: 9, color: td }}>
                {dayOvr != null ? "Manual override" : `${totalHours.toFixed(1)} hrs / ${team.length} crew`}
              </div>
            </div>
            <div>
          ...
          <div style={{ background: c2, borderRadius: 6, overflow: "hidden", border: `1px solid ${bd}`, marginBottom: 14 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: ff, fontSize: 11 }}>
              <thead><tr style={{ background: c2 }}>
                {["Task", "Auto Hrs", "Hours", ""].map(h =>
                  <th key={h} style={{ padding: "8px 10px", textAlign: h === "Task" ? "left" : "right", color: ac, fontSize: 10, borderBottom: `1px solid ${bd}`, fontWeight: 600 }}>{h}</th>
                )}
              </tr></thead>
              <tbody>{taskRows.map(t => {
                const isOvr = taskOvr[t.id] != null;
                return (
                  <tr key={t.id} style={{ borderBottom: `1px solid ${bd}08`, background: c1 }}>
                    <td style={{ padding: "6px 10px", color: tx }}>{t.nm}</td>
                    <td style={{ padding: "6px 10px", textAlign: "right", color: isOvr ? `${td}80` : td, textDecoration: isOvr ? "line-through" : "none", fontSize: 10 }}>
                      {t.auto}
                    </td>
                    <td style={{ padding: "6px 10px", textAlign: "right", width: 80 }}>
                      <input style={{ ...inp, padding: "4px 8px", fontSize: 11, textAlign: "right", width: 64, color: isOvr ? ac : tx }}
                        type="number" min={0} max={999} step={0.5}
                        value={isOvr ? taskOvr[t.id] : t.auto}
                        onChange={e => {
                          const v = +e.target.value;
                          setTaskOvr(o => ({ ...o, [t.id]: Math.max(0, v || 0) }));
                        }} />
                    </td>
                    <td style={{ padding: "6px 10px", textAlign: "center", width: 32 }}>
                      {isOvr && <button onClick={() => setTaskOvr(o => { const n = { ...o }; delete n[t.id]; return n; })}
                        style={{ background: "none", border: "none", color: td, cursor: "pointer", fontSize: 10, fontFamily: ff }}>reset</button>}
                    </td>
                  </tr>
                );
              })}</tbody>
              <tfoot><tr style={{ background: c2 }}>
                <td style={{ padding: "8px 10px", fontWeight: 700, color: ac }}>TOTAL</td>
                <td />
                <td style={{ padding: "8px 10px", textAlign: "right", fontWeight: 700, color: ac, fontSize: 12 }}>{totalHours.toFixed(1)} hrs</td>
                <td />
              </tr></tfoot>
            </table>
          </div>

          {/* ── Employee Roster (collapsible) ── */}
          <div style={{ marginBottom: 14 }}>
            <button onClick={() => setRosterOpen(o => !o)}
              style={{ background: "none", border: "none", cursor: "pointer", fontFamily: ff, fontSize: 10, fontWeight: 600, color: td, textTransform: "uppercase", letterSpacing: "0.08em", padding: "4px 0", display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ display: "inline-block", transform: rosterOpen ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.15s" }}>&#9654;</span>
              Saved Employee Database ({employees.length})
            </button>
            {rosterOpen && (
              <div style={{ borderRadius: 6, overflow: "hidden", border: `1px dashed ${td}60`, marginTop: 6 }}>
                <div style={{ background: `${td}10`, padding: "6px 10px", fontFamily: ff, fontSize: 10, color: td, borderBottom: `1px dashed ${td}60`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span>Persists across all projects — use <b>"+ Team"</b> to copy into this project</span>
                </div>
                <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: ff, fontSize: 11 }}>
                  <thead><tr style={{ background: c2 }}>
                    {["Name", "Skill", "$/hr", "", ""].map((h, i) =>
                      <th key={i} style={{ padding: "8px 10px", textAlign: h === "Name" || h === "Skill" ? "left" : "right", color: td, fontSize: 10, borderBottom: `1px solid ${bd}`, fontWeight: 600 }}>{h}</th>
                    )}
                  </tr></thead>
                  <tbody>{employees.map(emp => (
                    <tr key={emp.id} style={{ borderBottom: `1px solid ${bd}08`, background: c1 }}>
                      <td style={{ padding: "6px 10px" }}>
                        <input style={{ ...inp, padding: "4px 8px", fontSize: 11 }} value={emp.nm}
                          onChange={e => updEmployee(emp.id, "nm", e.target.value)} />
                      </td>
                      <td style={{ padding: "6px 10px" }}>
                        <select style={{ ...inp, padding: "4px 8px", fontSize: 11 }} value={emp.skill}
                          onChange={e => { updEmployee(emp.id, "skill", e.target.value); updEmployee(emp.id, "rate", SKILLS.find(s => s.id === e.target.value)?.rate || 25); }}>
                      ...
                      <td style={{ padding: "6px 10px", width: 80 }}>
                        <input style={{ ...inp, padding: "4px 8px", fontSize: 11, textAlign: "right" }}
                          type="number" min={10} max={200} step={1} value={emp.rate}
                          onChange={e => updEmployee(emp.id, "rate", +e.target.value || 25)} />
                      </td>
                      <td style={{ padding: "6px 10px", textAlign: "center" }}>
                        <button onClick={() => addToTeam(emp)}
                          style={{ ...bt(true), fontSize: 10, padding: "3px 8px", whiteSpace: "nowrap" }}>+ Team</button>
                      </td>
                      <td style={{ padding: "6px 10px", textAlign: "center", width: 32 }}>
                        <button onClick={() => delEmployee(emp.id)}
                          style={{ background: "none", border: "none", color: rd, cursor: "pointer", fontSize: 14, fontWeight: 700 }}>x</button>
                      </td>
                    </tr>
                  ))}</tbody>
                </table>
                <div style={{ padding: "6px 10px", background: `${td}10` }}>
                  <button onClick={addEmployee} style={{ ...bt(false), fontSize: 10, padding: "4px 10px" }}>+ Add Employee</button>
                </div>
              </div>
            )}
          </div>
          ...
          {/* ── Project Team ── */}
          <div style={{ fontFamily: ff, fontSize: 10, fontWeight: 600, color: ac, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>
            Project Team
            <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, fontSize: 10, color: td }}>— crew assigned to this job</span>
          </div>
          <div style={{ background: c2, borderRadius: 6, overflow: "hidden", border: `2px solid ${ac}40` }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: ff, fontSize: 11 }}>
              <thead><tr style={{ background: c2 }}>
        ...
        <div style={{ ...cd, marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
            <div style={hdr}>Grants & Incentives</div>
            <div style={{ flex: 1 }} />
            {grantTotal > 0 && <div style={{ fontFamily: ff, fontSize: 11, color: gn, fontWeight: 700 }}>-${grantTotal.toFixed(0)} total</div>}
          </div>

          {/* Quick-add presets grouped by category */}
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: grants.length > 0 ? 10 : 0 }}>
            {Object.entries(CAT_LABELS).map(([cat, catLb]) => {
              const items = GRANT_PRESETS.filter(p => p.cat === cat);
              if (!items.length) return null;
              return items.map(p => (
                <button key={p.id} onClick={() => addGrant(p)}
                  style={{ fontFamily: ff, fontSize: 9, padding: "3px 8px", borderRadius: 4, border: `1px solid ${bd}`, background: c2, color: tx, cursor: "pointer", whiteSpace: "nowrap" }}>
                  + {p.nm}
                </button>
              ));
            })}
          ...
          {/* Applied grants table */}
          {grants.length > 0 && (
            <div style={{ background: c2, borderRadius: 6, overflow: "hidden", border: `1px solid ${bd}` }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: ff, fontSize: 11 }}>
                <thead><tr style={{ background: c2 }}>
                  {["Incentive", "Type", "Value", "Amount", ""].map(h =>
                    <th key={h} style={{ padding: "6px 10px", textAlign: h === "Incentive" ? "left" : "right", color: ac, fontSize: 10, borderBottom: `1px solid ${bd}`, fontWeight: 600 }}>{h}</th>
                  )}
                </tr></thead>
                <tbody>{grantRows.map(g => (
                  <tr key={g.id} style={{ borderBottom: `1px solid ${bd}08`, background: c1 }}>
                    <td style={{ padding: "5px 10px" }}>
                      <input style={{ ...inp, padding: "3px 6px", fontSize: 11, width: "100%" }} value={g.nm}
                        onChange={e => updGrant(g.id, "nm", e.target.value)} />
                      {g.note && <div style={{ fontSize: 9, color: td, marginTop: 1 }}>{g.note}</div>}
                    </td>
                    <td style={{ padding: "5px 10px", textAlign: "right", width: 90 }}>
                      <select style={{ ...inp, padding: "3px 6px", fontSize: 10 }} value={g.tp}
                        onChange={e => updGrant(g.id, "tp", e.target.value)}>
                        <option value="pct">% of cost</option>
                        <option value="wpv">$/watt</option>
                        <option value="fix">Fixed $</option>
                      </select>
                    </td>
                    <td style={{ padding: "5px 10px", textAlign: "right", width: 80 }}>
                      <input style={{ ...inp, padding: "3px 6px", fontSize: 11, textAlign: "right", width: 64 }}
                        type="number" min={0} step={g.tp === "pct" ? 1 : g.tp === "wpv" ? 0.01 : 100}
                        value={g.val}
                        onChange={e => updGrant(g.id, "val", Math.max(0, +e.target.value || 0))} />
                    </td>
                    <td style={{ padding: "5px 10px", textAlign: "right", color: gn, fontWeight: 600, whiteSpace: "nowrap" }}>
                      -${g.amt.toFixed(0)}
                    </td>
                    <td style={{ padding: "5px 10px", textAlign: "center", width: 28 }}>
                      <button onClick={() => delGrant(g.id)}
                        style={{ background: "none", border: "none", color: rd, cursor: "pointer", fontSize: 14, fontWeight: 700 }}>x</button>
                    </td>
                  </tr>
                ))}</tbody>
                {grantRows.length > 1 && <tfoot><tr style={{ background: c2 }}>
                  <td colSpan={3} style={{ padding: "6px 10px", textAlign: "right", fontWeight: 700, color: gn, fontSize: 11 }}>TOTAL INCENTIVES</td>
                  <td style={{ padding: "6px 10px", textAlign: "right", fontWeight: 700, color: gn, fontSize: 12 }}>-${grantTotal.toFixed(0)}</td>
                  <td />
                </tr></tfoot>}
              </table>
            </div>
          ...
          {grants.length === 0 && (
            <div style={{ fontFamily: ff, fontSize: 10, color: td, marginTop: 4 }}>Click a preset above to add an incentive, or add "Other" for custom entries.</div>
          )}
        </div>
        ...
        <div style={{ ...cd, marginBottom: 14, background: `${ac}08`, border: `2px solid ${ac}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: grantTotal > 0 ? "1fr 1fr 1fr 1fr" : "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
            {[
              ["Material", matMarked, `$${matCost.toFixed(0)} + ${matMkup}%`],
              ["Labor", labMarked, `$${labCost.toFixed(0)} + ${labMkup}%`],
              ["Gross Total", totalCost, `$${totalProfit.toFixed(0)} profit`],
              ...(grantTotal > 0 ? [["Net After Incentives", netCost, `-$${grantTotal.toFixed(0)} in grants`]] : []),
            ].map(([label, val, sub]) => (
              <div key={label} style={{ textAlign: "center" }}>
                <div style={{ fontFamily: ff, fontSize: 10, color: td, textTransform: "uppercase", marginBottom: 4 }}>{label}</div>
                <div style={{ fontFamily: ff, fontSize: label === "Net After Incentives" ? 24 : label === "Gross Total" ? (grantTotal > 0 ? 18 : 24) : 20, fontWeight: 700, color: label === "Net After Incentives" ? gn : label === "Gross Total" ? ac : tx }}>
                  ${val.toFixed(0)}
                </div>
                <div style={{ fontFamily: ff, fontSize: 9, color: label === "Net After Incentives" ? gn : label === "Gross Total" ? gn : td }}>{sub}</div>
              </div>
            ))}
          </div>
          <div style={{ borderTop: `1px solid ${bd}`, paddingTop: 12, display: "flex", justifyContent: "space-around", fontFamily: ff, fontSize: 11 }}>
            ...
            {grantTotal > 0 && <div><span style={{ color: td }}>Net $/W: </span><span style={{ color: gn, fontWeight: 700 }}>${netPerWatt.toFixed(2)}</span></div>}
        ...
        <div style={{ ...cd }}>
          ...
          <div style={{ background: c2, borderRadius: 6, padding: 16, fontFamily: ff, fontSize: 11, lineHeight: 1.7 }}>
            ...
            <div style={{ borderTop: `1px solid ${bd}`, paddingTop: 8, marginBottom: 8 }}>
              ...
              {grantTotal > 0 && <>
                {grantRows.map(g => (
                  <div key={g.id} style={{ display: "flex", justifyContent: "space-between", marginTop: 3, fontSize: 10, color: gn }}>
                    <span>{g.nm}</span><span>-${g.amt.toFixed(2)}</span>
                  </div>
                ))}
                <div style={{ display: "flex", justifyContent: "space-between", borderTop: `2px solid ${gn}`, paddingTop: 6, marginTop: 6 }}>
                  <span style={{ fontWeight: 700, color: gn, fontSize: 13 }}>NET COST</span>
                  <span style={{ fontWeight: 700, color: gn, fontSize: 13 }}>${netCost.toFixed(2)}</span>
                </div>
              </>}
              <div style={{ display: "flex", gap: 16, marginTop: 6, fontSize: 10, color: td }}>
                <span>$/Watt: <b style={{ color: ac }}>${pricePerWatt.toFixed(2)}</b></span>
                {grantTotal > 0 && <span>Net $/W: <b style={{ color: gn }}>${netPerWatt.toFixed(2)}</b></span>}
